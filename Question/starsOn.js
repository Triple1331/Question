...
let root = null, dodecahedronRadius



function createSceneA() {
    dodecahedronRadius = 15;
    nbrBursts = 400;
    maxRays = 100; 
    maxRad = 1;
    root = starburstsOnDodecahedr(nbrBursts, dodecahedronRadius, maxRays, maxRad)
    scene.add(root);
}


function starburstsOnDodecahedr(nbrBursts, dodecahedronRadius, maxRays, maxRad) {
    let root = new THREE.Object3D();
    for (let i = 0; i < nbrBursts; i++) {
        let mesh = starburstA(maxRays, maxRad);
        let p = getRandomPointOnDodecahedr(dodecahedronRadius);
...
function update() {
    ...
    if (root)
        scene.remove(root);
    root = starburstsOnDodecahedr(nbrBursts, dodecahedronRadius, maxRays, burstRadius);
    scene.add(root);
}
